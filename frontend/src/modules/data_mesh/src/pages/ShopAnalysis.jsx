...
function DataProductReadiness({ shopId }) {
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      axios.get(`${API_BASE}/domains/metadata`),
      axios.get(`${API_BASE}/domain-health/anomalies`)
...
export default function ShopAnalysis() {
  ...
  useEffect(() => {
    ...
    axios.get(`${API_BASE}/api/shop-overview`).then(res => setOverview(res.data));
  }, []);
  ...
  useEffect(() => {
    ...
    setLoading(true);
    axios.get(`${API_BASE}/sales?shop_id=${selectedShop}`).then(res => {
